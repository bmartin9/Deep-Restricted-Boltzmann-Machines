        ...
        with torch.no_grad():
            ...
            if option == False:
                ...
                for layer in range(self.numlayers-1):
                    ...
                    weight_gradients[layer] = torch.where(weight_mask[layer]==100,torch.zeros(weight_mask[layer].size()),weight_gradients[layer]) #set the block connections to 1000
                    ...
                    bias_gradients[layer] = torch.where(bias_mask[layer]==-10,torch.zeros(bias_mask[layer].size()),bias_gradients[layer]) #set the non trainable visible bias derivatives to zero
                    self.weights[layer].grad = -weight_gradients[layer]
                    self.bias[layer].grad = -bias_gradients[layer]
                ...
                bias_gradients[-1] = torch.where(bias_mask[-1]==-10,torch.zeros(bias_mask[-1].size()),bias_gradients[-1])
                self.bias[-1].grad = -bias_gradients[-1]
